...
## [1.2.1] - 2021-02-20
### Added
- Performance enhancement per production usage and customer requirements:
  - Add extra metric (request per second) in result page
  - Add extra CNAME option and association in CloudFormation template for local regulation
  - Add timeout configuration in underlay test framework to avoid infinite loop in large concurrence test
  
### Changed
- Workaround & mitigation for missing features in ZHY/BJS:
  - Cognito (user pool) missing, resolved by decoupling legacy login page and application page, integrate with OIDC per customer
  - CodePipeline missing, resolved by constructing docker image locally and archived to unified AWS gallery
  - CloudWatch API (getMetricWidgetImage) missing, resolved by add extra button in application page redirecting to CloudWatch console
  - Promote legacy concurrent virtual user limit over 18+ times (from 200*100 to 750*500)
  - Chinese localization in application and result page
